void main() {
  ...
  set.add(123);
  print(set);
  var testFollowedBy = testList6.followedBy(set);
...
//   print("删除列表中toString后长度为3的元素：==> $testList6");
  ...
  //第二个参数可以指定查找的范围
  print(testList6.indexOf(34532, 5));

  //遍历查询有没有满足条件的数据，如果有就返回true，没有就返回false
  //参数传入一个返回bool的函数
  var testExit = testList6.any((item) => item == 555);
  print("textExit==>$testExit");

  //遍历查询是不是所有的数据都满足这个条件，如果有不满足的数据，直接返回false
  bool testEvery = testList6.every((item) => item == 555);
  ...
  //就说明这个指定的位置有问题。
  var position = testList6.elementAt(testList6.length - 2);
  print("position ====> $position");

  //可以理解为将一个一维数组扩展为二维数组
  print(testList2);
  var testList9 =
      testList2.expand((item) => [item, item.toString() + "啊"].toList());
  print("test expand ===> $testList9");

  //同时也可已经一个二维数组解封,解封时需要数组里面的item仍然是一个list
  var testList10 = [
    [12, 12],
    [22, 44]
  ];
  ...
  testList6.add(1);
  var testSingleWhere =
      testList6.singleWhere((item) => item.toString().length == 7, orElse: () {
    return 'siquba';
  });
  print(
  ...
  print(testList6);
  var testTakeWhile =
      testList6.takeWhile((item) => item.toString().length == 3);
  print(
  ...
  var list = ['哼哼哼', ' 嘎嘎嘎', ' 哼哼哼'];
  print(list);
  String str = list.fold("哈哈哈 ", (pre, item) => pre + item);
  print("test fold ====> $str");
  ...
  print("${testList6}");
  var testReduce = testList6.reduce(
      (value, element) => checkReduce(value.toString(), element.toString()));
...
checkReduce(String string, String string2) {
